...
2.  **GPU Воркер**: Демонстрирует «тяжелые» задачи с **отчетами о прогрессе**, **горячим кэшем** и **автоматической загрузкой файлов в S3**. Пакет `avtomatika-worker`.
...
Интеграция `actions.await_human_approval()`. Пайплайн приостанавливается на старте, переходя в статус `waiting_for_human` до получения внешнего решения `APPROVED` через API.

### 2. Умное распределение задач
*   **Ресурсные ограничения**: Задачи, требующие специфического CPU/RAM (логика GE - Greater or Equal).
*   **Matching по Hot Cache**: Использование `resource_hint` для выбора воркеров, у которых уже загружены нужные ресурсы (например, AI-модели).
...
### 3. Расширенное взаимодействие с воркерами
...
*   Каждое сообщение криптографически подписано.
*   Оркестратор проверяет подписи всех акторов, включая внутренние события «призраков» (Ghost) из блюпринтов.
*   Защита от повторов (Replay protection) через обязательные таймштампы.

### 5. Современный синтаксис блюпринтов
...
*   **Параллелизм**: Простой `fan-out / fan-in` через `actions.dispatch_parallel()`.
...
*   `workers/gpu.py`: Продвинутый воркер с прогрессом, событиями и горячим кэшем.
